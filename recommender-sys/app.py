...
class recommend(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            user_activity = data['activity_type']
            ...
            user_data['age'], user_data['antiguedad'], user_data['ind_actividad_cliente'], user_data['renta']\
            ,user_data['segmento'] = data['age'], data['seniority'],\
                data['client_activity'], data['income'],data['segment']
    
            '''
            generating the recommendations from the model
            feeding our client's data to the model to get the predictions
            '''
            x_vars_list, y_vars_list, cust_dict = processData([data], {}, False)
